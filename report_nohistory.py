...
def parse_experiment(path, coverage_dir='coverage'):
    ...
    cov = Coverage()
    files = os.listdir(coverage_path)
    for covfile in files:
        if('.json' not in covfile and covfile != '.DS_Store'):
            tmp = Coverage()
            with open(os.path.join(coverage_path, covfile), 'rb') as f:
                tmp.load(f)
            cov |= tmp

    return experiment, command, runtime, cov
...
if __name__ == '__main__':
    ...
    idx = defaultdict(int)
    with open('out_nohist.csv', 'w') as f:
        out = csv.writer(f)
        ...
        for app in os.listdir(experiments_dir):
            ...
            for crawler in os.listdir(app_dir):
                ...
                with Pool(processes=20) as pool:
                    for experiment, command, runtime, cov, in tqdm(pool.map(parse_experiment, experiment_dirs)):
                        algo = command.split(' ')
